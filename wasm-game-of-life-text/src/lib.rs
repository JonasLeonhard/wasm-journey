...
#[wasm_bindgen]
impl Universe {
    pub fn new(width: u32, height: u32, canvas: HtmlCanvasElement) -> Universe {
    ...
    pub fn draw_grid(&self) {
        self.canvas
            .set_height((self.cell_size as u32 + 1) * self.height + 1);
        self.canvas
            .set_width((self.cell_size as u32 + 1) * self.width + 1);

        self.ctx.begin_path();
        self.ctx.set_stroke_style(&self.grid_color);

        (0..self.width).for_each(|i| {
            self.ctx
                .move_to(i as f64 * (&self.cell_size + 1.0) + 1.0, 0.0);
            self.ctx.line_to(
                i as f64 * (&self.cell_size + 1.0) + 1.0,
                (&self.cell_size + 1.0) * self.height as f64 + 1.0,
            );
        });

        (0..self.height).for_each(|i| {
            self.ctx
                .move_to(0.0, i as f64 * (&self.cell_size + 1.0) + 1.0);
            self.ctx.line_to(
                (&self.cell_size + 1.0) * self.width as f64 + 1.0,
                i as f64 * (&self.cell_size + 1.0) + 1.0,
            );
        });
    ...
    pub fn draw_cells(&self) {
        ...
        (0..self.height).for_each(|row| {
            (0..self.width).for_each(|col| {
                ...
                self.ctx.fill_rect(
                    col as f64 * (&self.cell_size + 1.0) + 1.0,
                    row as f64 * (&self.cell_size + 1.0) + 1.0,
                    self.cell_size,
                    self.cell_size,
                );
            });
